...
public class MapWebServer {
    ...
    private HttpServer listen(String host, int port, Executor pool) throws IOException {
        ...
        server.createContext("/api/resync", wrap(terrain::resync));
        server.createContext("/api/terminals", wrap(terrain::terminals));
        ...
        server.createContext("/api/perf", wrap(terrain::perf));
